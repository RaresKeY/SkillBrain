...
from fleet_game.core import GameEngine
from fleet_game.agents import BacktrackingAgent, BlankAgent, CheatingRAgent, GreedyAgent, PruningBacktrackingAgent, RAgent, StudentAgent
from fleet_game.models import Route
from fleet_game.ui.constants import *
from fleet_game.ui.map_renderer import MapRenderer
from fleet_game.ui.components import Button, Panel, CollapsiblePanel, StatDisplay, RadioButton, Tooltip
from fleet_game.ui.font_utils import render_text
from fleet_game.ui.manual_mode import ManualModeManager
from fleet_game.utils.metrics import calculate_route_metrics
...
class DeliveryFleetApp:
    ...
    def _register_agents(self):
        ...
        self.engine.register_agent("agent_r_cheating", CheatingRAgent(self.engine.delivery_map))
        self.engine.register_agent("blank_agent", BlankAgent(self.engine.delivery_map))
        self.engine.register_agent("agent_r", RAgent(self.engine.delivery_map))
        try:
            from fleet_game.agents import NeuralRAgent
    ...
    def render_map(self):
        ...
        if self.mode == "MANUAL" and self.manual_mode_manager:
            ...
            for i, (veh, packages, stops) in enumerate(route_data):
                ...
                from fleet_game.models.route import Route
                temp_route = Route(
                    vehicle=veh,
                    packages=packages,
                    stops=stops,
