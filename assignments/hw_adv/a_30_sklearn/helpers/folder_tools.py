...
def is_safe_path(path_to_check, block_list: set[str] = BLOCK_LIST, whitelist=None) -> bool:
    """
        ...
        for blocked in block_list:
            if p_str.startswith(blocked.lower()):
                if blocked in whitelist:
                    print(f"INFO: {blocked} allowed override via passed whitelist")
                    continue
                print(f"Error: {blocked} path part not allowed")
                return False
...
def file_generator(folder_path: Path, types: ft, whitelist: None | list = None):
    # Ensure the path exists first
    if not folder_path.exists():
        print(f"Error: The folder '{folder_path}' does not exist.")
        return iter(())

    if not is_safe_path(folder_path, whitelist=whitelist):
    ...
    if ft.JSON in types:
        selected_exts.update({".json", ".jsonl"})
